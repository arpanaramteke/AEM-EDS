export default function decorate(block) {
  // 1. State (keep inside block scope)
  const stocks = {
  ...
  // 2. Create container (instead of getElementById)
  const tickerTrack = document.createElement('div');
  tickerTrack.className = 'ticker-track';

  // 3. Create stock DOM node
  ...
  // 5. Update logic (NO global querySelector)
  function updateStockPrice(symbol, newPrice) {
    ...
    // IMPORTANT: scope to block (EDS best practice)
  ...
  // 7. Simulation (replace with WebSocket later)
  setInterval(() => {
